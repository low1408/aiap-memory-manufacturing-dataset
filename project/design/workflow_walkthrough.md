...
|---|---|---|
| [Client brief](../brief/helion_semiconductor_client_brief.md) | Objective, procedure catalogue, nominal times, inconclusive rates, four Yield Engineering owners and independent audits | Actual full SOP, dollar rates or validated procedure outcomes |
| [MOCK-ENG-002](mock_engineering_inspection_rules.md) | Candidate concern-opening rules, co-fault safeguards, priority order and routine stopping standard | Helion's actual approved procedure or closure standard |
...
2. Apply candidate triggers, failed-acceptance branches and co-fault safeguards. Determine eligible procedures and preserve intact-sample evidence before destructive work.
...
5. Update only questions supported by the report's scope. Reconsider the next test; a nondestructive procedure may repeat once after an inconclusive attempt. Routine closure requires all opened concerns and failed branches to be resolved or explicitly escalated; independent audits still require all-seven evidence.
...
| Branch A1: qualified W positive, V negative, gross-D negative | Record W present and V absent; gross-D negative cannot exclude all D | The opened warpage concern is resolved; other mechanisms remain untested rather than absent |
| Branch A2: qualified W/V negative, gross-D negative | Record W/V absent | The unexplained assembly branch continues through the qualified follow-up path |
| Branch A3: procedure inconclusive | W/V/D remain unresolved | $120 still spent; one conditional CT repeat or a qualified alternative remains eligible |

Branches are alternative reports, not three attempts. Under SYN-OPS-001, an inconclusive event is shared across questions examined by that procedure. After A1, a routine case may be concern-complete even though D/E/T/M/C remain untested; only an independent audit requires the full battery. After A2, the assembly rejection remains unexplained and follows the CT–acoustic–SEM escalation. A3 permits one independently drawn CT repeat because the first attempt was inconclusive.
...
Both acoustic and electrical isolation are justified. **MOCK-ENG-002 chooses acoustic first** because its documented concern-priority order puts acoustic before electrical among simultaneously eligible procedures. The brief's electrical example does not establish a universal electrical-first rule. CT has no unresolved W/V concern here and is not a repeat candidate because its report was conclusive.
...
|---|---:|---|
| Prior CT: W/V absent | $120 | D and the specific interconnect branch |
| Acoustic: D present; no contradictory V finding | $320 | TSV/microbump differential |
| Electrical: E absent, T present | $920 | Microbump co-fault safeguard |
| SEM remains a required diagnostic continuation | $920 spent; $1,800 planned | Microbump remains pending; crack stays untested unless separately indicated |
...
Continue Case B's conclusive branch after the engineer preserves all required nondestructive evidence and authorises SEM. Its low initial microbump probability, say 0.05, cannot remove the R09 co-fault concern. Assume a scoped conclusive SEM report confirms M and T and excludes C, without contradictions.
...
|---|---|---|
| Baseline / closure | MOCK-ENG-002 concern closure with seven separately recorded states | Actual SOP, concern-retirement authority, exceptions, closure and repeat standard; quality owner |
...
**Decision experiment specified in this paper exercise:** exercise the candidate rules and coverage heuristic against identical diagnostic and cost assumptions. Use scripted branches before stochastic simulation. Fault priors are not procedure outcome labels; synthetic sensitivities are not a trained outcome predictor. Store hidden truth separately from policy inputs, preserve coexisting faults, and allow only the configured independent repeat after an inconclusive nondestructive attempt. Unspecified continuation branches end in explicit unresolved/escalated states, not invented cost-free completion.

| Arm | Diagnostic selection |
|---|---|
| A | Candidate engineering rules and their concern-priority order |
| B | Model-assisted cost-aware next-test policy |
| C | Simple fixed-order alternative with the same evidence rules |

Hold the permitted procedures, concern-opening and stopping standards, costs and report assumptions constant. Include a rules-plus-evidence-checklist alternative so workflow discipline is not credited to ML. [Comparison design](P1_ml_systems_helion.ipynb#helion-evaluation)
