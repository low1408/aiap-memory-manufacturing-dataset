# Archived v1 dataset description and pre-pipeline proposal

> **Archived.** This is the repository README as it stood before the `HELION-PIPELINE-002` research run.
> Statements that no model has been trained, that no workflow comparison exists or that Cores 3–4 are pending are
> **superseded**. See the [current README](../../README.md) and the
> [v2 benchmark report](../../artifacts/helion_pipeline/research_v2_concern_closure/benchmark_report.md).
> The dataset description (record structure, generation, missingness, scope of realism) remains accurate for `data/`.
...
[project/evidence/SOURCES.md](SOURCES.md).
...
[BYO_PROBLEM.md](../../../BYO_PROBLEM.md), while retaining the delivered v1 data description.
...
counts in the archived v1 [validation report](VALIDATION_REPORT.md) include
...
generating rules in `project/evidence/GENERATING_MODEL.md`. Generator source references are
...
test selection. The archived [baseline report](BASELINE_REPORT.md),
[baseline metrics](../../metadata/baseline_metrics.json) and
[review decision audit](REVIEW_DECISION_AUDIT.md) retain their v1 scope.
...
The retrospective [rule audit](REVIEW_DECISION_AUDIT.md) compares the archived
...
Reference: [Bring Your Own Problem - Pre-Work Brief](../../../BYO_PROBLEM.md), Sections
...
This copy is organized for data analysis rather than regeneration. Project-facing
material is grouped under `project/` so that source authority and lifecycle stay
clear:
...
project/brief/         Supplied client brief and assessment rubric
project/design/        Design notebook, architecture, rules and editable diagrams
project/presentation/  Pre-read, presentation outline, viva preparation and slides
project/evidence/      Archived v1 source, validation, baseline and audit reports
